                                          ...
                                          const CHAR *file_name_ptr)
{
  ...
  else
  {
    ...
    for (shift_index = entry_capacity - 1U; shift_index > insert_pos; shift_index--)
    {
      entry_list_ptr[shift_index] = entry_list_ptr[shift_index - 1U];
    }
  }

  entry_list_ptr[insert_pos].snapshot_index = snapshot_index;
  (void)snprintf((char *)entry_list_ptr[insert_pos].file_name,
                 sizeof(entry_list_ptr[insert_pos].file_name),
                 "%s",
                 file_name_ptr);
