...
static uint8_t g_current_pseudo_index = 4U;
static uint8_t g_current_gain_high = 1U;
static uint8_t g_current_auto_ffc = 1U;
static uint8_t g_current_alarm_enable = 1U;
static uint8_t g_current_unit_celsius = 1U;
static uint8_t g_current_sleep_index = 2U;
...
static void thermal_gui_refresh_device_info(lv_ui *ui)
{
    ...
    (void)snprintf(summary,
                   ...
                   (unsigned)g_current_sleep_index,
                   ...
                   (unsigned)g_current_alarm_enable,
                   (unsigned)g_config_saved);
    lv_label_set_text(device_info_label, summary);
}

/**
 * @brief Reset UI state to the project defaults.
 * @param ui UI context.
 * @return None.
 */
static void thermal_gui_reset_state_to_default(lv_ui *ui)
{
    g_current_pseudo_index = 4U;
    g_current_gain_high = 1U;
    g_current_auto_ffc = 1U;
    g_current_alarm_enable = 1U;
    g_current_unit_celsius = 1U;
    g_current_sleep_index = 2U;
    g_current_contrast_slider = 54U;
    g_current_mirror_enable = 0U;
    g_current_flip_enable = 0U;
    g_current_center_cross_enable = 1U;
    g_current_center_temp_enable = 1U;
    g_current_hi_lo_mark_enable = 1U;
    g_config_saved = 0U;

    (void)tiny1c_vdcmd_set_pseudo_color(PREVIEW_PATH0, PSEUDO_COLOR_MODE_5);
...
/**
 * @brief Handle restore-default button clicks.
 * @param e LVGL event descriptor.
 * @return None.
 */
static void thermal_gui_restore_default_event_cb(lv_event_t *e)
{
    if (lv_event_get_code(e) == LV_EVENT_CLICKED)
    {
        (void)tiny1c_vdcmd_restore_all_defaults();
        thermal_gui_reset_state_to_default(&guider_ui);
    }
}
...
static void thermal_gui_sleep_event_cb(lv_event_t *e)
{
    lv_event_code_t code = lv_event_get_code(e);
    ...
    if (code == LV_EVENT_VALUE_CHANGED)
    {
        g_current_sleep_index = (uint8_t)lv_dropdown_get_selected(dd);
    }
}

/**
 * @brief Handle the alarm switch.
 * @param e LVGL event descriptor.
 * @return None.
 */
static void thermal_gui_alarm_event_cb(lv_event_t *e)
{
    lv_event_code_t code = lv_event_get_code(e);
    lv_obj_t *sw = lv_event_get_target(e);
    ...
    if (code == LV_EVENT_VALUE_CHANGED)
    {
        g_current_alarm_enable = (lv_obj_has_state(sw, LV_STATE_CHECKED) != 0U) ? 1U : 0U;
    }
}
...
void events_init_WidgetsDemo(lv_ui *ui)
{
    ...
    lv_obj_add_event_cb(ui->WidgetsDemo_btn_restore_cfg, thermal_gui_restore_default_event_cb, LV_EVENT_CLICKED, NULL);
    ...
    lv_obj_add_event_cb(ui->WidgetsDemo_ddlist_sleep, thermal_gui_sleep_event_cb, LV_EVENT_VALUE_CHANGED, NULL);
    lv_obj_add_event_cb(ui->WidgetsDemo_sw_alarm, thermal_gui_alarm_event_cb, LV_EVENT_VALUE_CHANGED, NULL);
