...
#define CFG_THERMAL_GUI_GALLERY_MAX_ENTRIES      32U
#define CFG_THERMAL_GUI_GALLERY_NAME_STRIDE      CFG_APP_FILEX_SNAPSHOT_NAME_LEN
...
static CHAR g_snapshot_gallery_names[CFG_THERMAL_GUI_GALLERY_MAX_ENTRIES][CFG_THERMAL_GUI_GALLERY_NAME_STRIDE];
...
{
    return app_filex_snapshot_get_list((CHAR *)g_snapshot_gallery_names,
                                       CFG_THERMAL_GUI_GALLERY_MAX_ENTRIES,
                                       CFG_THERMAL_GUI_GALLERY_NAME_STRIDE,
...
static UINT thermal_gui_gallery_load_entry(uint32_t entry_index)
{
    ...
    if ((g_snapshot_gallery_count == 0U) || (entry_index >= g_snapshot_gallery_count))
    {
    ...
    status = app_filex_snapshot_load_rgb565(g_snapshot_gallery_names[entry_index],
    ...
    g_snapshot_gallery_index = entry_index;
    ...
    (void)snprintf(title_text,
                   sizeof(title_text),
                   "%s  (%lu/%lu)",
                   g_snapshot_gallery_names[entry_index],
...
static void thermal_gui_gallery_prev_event_cb(lv_event_t *e)
{
    ...
    {
        (void)thermal_gui_gallery_load_entry(g_snapshot_gallery_index - 1U);
    }
}
...
static void thermal_gui_gallery_delete_event_cb(lv_event_t *e)
{
    ...
    status = app_filex_snapshot_delete(g_snapshot_gallery_names[g_snapshot_gallery_index]);
    if (status != FX_SUCCESS)
    {
    ...
    status = thermal_gui_gallery_reload_list();
    if ((status == FX_SUCCESS) && (g_snapshot_gallery_count > 0U))
    {
        uint32_t next_index = g_snapshot_gallery_index;
        if (next_index >= g_snapshot_gallery_count)
        {
            next_index = g_snapshot_gallery_count - 1U;
        }
        (void)thermal_gui_gallery_load_entry(next_index);
    }
    else
    {
        lv_label_set_text(g_snapshot_gallery_status, "No snapshot saved");
...
void custom_init(lv_ui *ui)
{
    ...
    lv_obj_set_pos(g_snapshot_status_label, 136, 154);
    ...
    lv_obj_set_pos(ui->WidgetsDemo_btn_device_info, 8, 194);
