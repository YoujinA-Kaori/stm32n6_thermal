...
#define CFG_TINY1C_FRAME_WIDTH             160U
#define CFG_TINY1C_FRAME_HEIGHT            120U
#define CFG_TINY1C_SCALE_FACTOR            2U
...
/**
 * @brief 将 8bit RGB888 通道值压缩为 RGB565。
 * @param red8 红色通道值，范围 0~255。
 * @param green8 绿色通道值，范围 0~255。
 * @param blue8 蓝色通道值，范围 0~255。
 * @return uint16_t：RGB565 像素值。
 */
/**
...
/**
 * @brief 将 Y14 温度值映射为 RGB565 伪彩像素。
 * @param temp14_value 当前像素温度值，14bit。
 * @param temp14_min 当前显示范围下限。
 * @param temp14_max 当前显示范围上限。
 * @return uint16_t：RGB565 伪彩像素值。
 */
...
/**
 * @brief 基于相邻 4 个温度点生成 2x 放大块内的插值温度值。
 * @param temp00 左上角温度值。
 * @param temp10 右上角温度值。
 * @param temp01 左下角温度值。
 * @param temp11 右下角温度值。
 * @param sub_x 目标点在 2x 块内的 x 偏移，只允许 0 或 1。
 * @param sub_y 目标点在 2x 块内的 y 偏移，只允许 0 或 1。
 * @return uint16_t 插值后的温度值。
 */
...
/**
 * @brief 保证显示范围跨度不小于最小值，避免小波动被过度放大。
 * @param temp14_min 显示范围下限指针。
 * @param temp14_max 显示范围上限指针。
 * @return 无。
 */
...
/**
 * @brief 对目标显示边界做慢速跟踪，减轻 FFC 后的突变观感。
 * @param current 当前边界值。
 * @param target 目标边界值。
 * @return uint16_t 平滑后的边界值。
 */
...
/**
 * @brief 根据整帧温度分布计算主体显示区间，忽略少量极端冷热像素。
 * @param frame_temp14 指向温度帧缓冲区。
 * @param frame_temp14_min 当前帧最小温度值。
 * @param frame_temp14_max 当前帧最大温度值。
 * @param disp_temp14_min 输出显示下限。
 * @param disp_temp14_max 输出显示上限。
 * @return 无。
 */
...
/**
 * @brief 选择当前显示使用的温度范围。
 * @param frame_temp14_min 当前帧推荐显示下限。
 * @param frame_temp14_max 当前帧推荐显示上限。
 * @param disp_temp14_min 显示范围最小值输出指针。
 * @param disp_temp14_max 显示范围最大值输出指针。
 * @return 无。
 */
...
/**
 * @brief 将采集到的 Y16 温度帧解码为 Y14，并进行 2 倍放大后显示到 LCD。
 * @param 无。
 * @return 无。
 */
...
/**
 * @brief DCMIPP 帧完成事件回调入口。
 * @param dcmipp_handle DCMIPP 句柄指针。
 * @param pipe 管道编号。
 * @return 无。
 */
